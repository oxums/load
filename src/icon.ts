export interface IconEntry {
  key: string;
  svg: string;
  description?: string;
  colorHint?: string;
}

type IconMap = Record<string, IconEntry>;

export const ICON_SIZE = 12;
export const ICON_SIZE_ALT = ICON_SIZE * 0.88;

const PLACEHOLDER_SVG = (label: string) =>
  `<svg viewBox="0 0 16 16" aria-label="${label}" xmlns="http://www.w3.org/2000/svg">
    <rect x="1" y="2" width="14" height="12" rx="2" ry="2" fill="currentColor" opacity="0.15"/>
    <text x="50%" y="55%" dominant-baseline="middle" text-anchor="middle" font-family="monospace" font-size="6" fill="currentColor">${label.slice(0, 3).toUpperCase()}</text>
  </svg>`;

export const ICONS: IconMap = {
  folder: {
    key: "folder",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Material Symbols by Google - https://github.com/google/material-design-icons/blob/master/LICENSE --><path fill="currentColor" d="M4 20q-.825 0-1.412-.587T2 18V6q0-.825.588-1.412T4 4h6l2 2h8q.825 0 1.413.588T22 8v10q0 .825-.587 1.413T20 20z"/></svg>`,
    description: "Folder",
    colorHint: "#cfa043",
  },
  file: {
    key: "file",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Google Material Icons by Material Design Authors - https://github.com/material-icons/material-icons/blob/master/LICENSE --><path fill="currentColor" d="M6 2c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm7 7V3.5L18.5 9z"/></svg>`,
    description: "File",
    colorHint: "#8893a2",
  },
  unknown: {
    key: "unknown",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Google Material Icons by Material Design Authors - https://github.com/material-icons/material-icons/blob/master/LICENSE --><path fill="currentColor" d="M6 2c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm7 7V3.5L18.5 9z"/></svg>`,
    description: "Unknown",
    colorHint: "#666",
  },

  // Common code / markup
  ts: {
    key: "ts",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M0 0v512h512V0zm294.445 222.23H221.63v225.553h-40.1V222.23h-72.665v-37.298h185.581zm149.212 202.559c-24.443 31.85-94.558 36.713-135.325 10.214v-49.857c22.19 24.235 45.152 27.699 63.042 29.342c55.89 4.65 57.509-41.905 34.886-61.101c-33.311-26.885-99.65-41.844-97.578-99.938c-.482-41.618 31.829-69.556 79.912-72.152c18.22-.984 44.193.15 60.527 10.698v46.867c-38.336-35.174-118.13-25.74-98.781 26.47c17.512 32.235 77.733 35.405 103.076 86.453c10.755 28.718 4.768 54.075-9.759 73.004"/></svg>`,
    description: "TypeScript",
    colorHint: "#3178c6",
  },
  tsx: {
    key: "tsx",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M454.528.001H57.469C27.102-.228.403 23.534 0 57.471v397.058c.404 33.937 27.102 57.698 57.47 57.47h397.058c39.276-.56 56.791-30.223 57.472-57.47V57.471C511.32 30.224 493.805.56 454.528 0M170.433 308.45h-48.83v155.887H82.417V308.45h-48.83v-39.185h136.845zm114.01 149.193c-31.375 13.921-87.05 8.847-106.163-33.191l31.487-18.193c15.744 29.913 56.577 29.842 64.593 10.14c7.829-19.236-15.787-26.758-45.095-39.755c-12.747-6.574-32.472-16.927-38.392-42.224c-4.467-19.09-.481-47.937 29.388-61.11l.004.002c41.429-15.204 73.237 4.198 82.567 26.59l-30.09 19.592c-4.448-8.028-13.928-16.237-24.873-16.824c-9.926-.533-19.154 4.657-19.448 18.08c.589 19.56 26.308 19.557 57.15 39.094c22.594 14.313 26.826 25.253 28.915 45.026c1.548 14.655-1.209 39.977-30.044 52.773m152.738 6.693l-37.786-67.173l-37.785 67.174h-38.485l54.579-100.06l-51.08-95.164l.002-.001h39.184l33.586 62.275l33.587-62.275h38.485l-50.38 95.163l54.578 100.06z"/></svg>`,
    description: "TSX",
    colorHint: "#3178c6",
  },
  js: {
    key: "js",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 16 16"><!-- Icon from Fluent UI System Icons by Microsoft Corporation - https://github.com/microsoft/fluentui-system-icons/blob/main/LICENSE --><path fill="currentColor" d="M10.77 7.3h.002c1.045.393 2.479.93 2.479 2.45a2.5 2.5 0 0 1-.224 1.02a2.5 2.5 0 0 1-1.515 1.364a2.5 2.5 0 0 1-1.035.115a2 2 0 0 1-.214.012a2.5 2.5 0 0 1-1.673-.65a2.52 2.52 0 0 1-.838-1.859c0-.202.078-.39.22-.532a.77.77 0 0 1 1.06 0a.74.74 0 0 1 .221.53c0 .952 1.041 1 1.25 1s1.25-.048 1.25-1c0-.413-.447-.648-1.514-1.048h-.003C9.19 8.307 7.753 7.77 7.753 6.25q.005-.537.224-1.02a2.5 2.5 0 0 1 .614-.842a2.5 2.5 0 0 1 .9-.52a3.5 3.5 0 0 1 2.023 0a2.52 2.52 0 0 1 1.738 2.381c0 .201-.078.39-.22.531a.77.77 0 0 1-1.061 0a.74.74 0 0 1-.22-.53c0-.952-1.041-1-1.25-1s-1.25.048-1.25 1c0 .413.447.648 1.514 1.048zM5.751 4.5c0-.2.078-.388.22-.53a.77.77 0 0 1 1.06 0c.142.141.22.33.22.53v5a2.75 2.75 0 0 1-4.695 1.945A2.73 2.73 0 0 1 1.75 9.5V9c0-.2.078-.388.22-.53a.77.77 0 0 1 1.061 0c.142.141.22.33.22.53v.5c0 .33.134.652.366.884c.465.465 1.303.465 1.768 0c.232-.233.366-.555.366-.884z"/></svg>`,
    description: "JavaScript",
    colorHint: "#f7df1e",
  },
  jsx: {
    key: "jsx",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M454.528.002H57.47C27.103-.228.404 23.534.001 57.471v397.058c.404 33.937 27.102 57.698 57.469 57.469h397.058c39.276-.56 56.791-30.222 57.472-57.469V57.471C511.32 30.224 493.805.56 454.528.002M155.886 415.331c-3.998 29.688-22.571 49.07-56.68 50.031c-26.581.75-48.166-10.729-61.578-36.037l32.189-19.593c.005.01 8.486 15.307 17.58 18.456c4.805 1.664 14.87 2.274 21.43-2.012c5.992-3.913 7.873-14.344 7.873-14.344v-141.35h39.186zm128.558 42.286c-31.375 13.922-87.052 8.848-106.166-33.191l31.488-18.194c15.745 29.915 56.579 29.843 64.595 10.141c7.83-19.238-15.788-26.76-45.097-39.757c-12.747-6.574-32.472-16.927-38.393-42.226c-4.467-19.09-.48-47.938 29.39-61.112l.003.002c41.43-15.204 73.24 4.199 82.57 26.59l-30.09 19.594c-4.449-8.028-13.93-16.237-24.875-16.824c-9.926-.533-19.154 4.656-19.449 18.08c.59 19.56 26.31 19.558 57.153 39.096c22.595 14.313 26.826 25.253 28.916 45.027c1.548 14.656-1.21 39.979-30.045 52.774m152.743 6.694l-37.786-67.176l-37.787 67.177h-38.486l54.58-100.064l-51.082-95.167h39.189l33.587 62.277l33.587-62.278h38.486l-50.382 95.167l54.58 100.064z"/></svg>`,
    description: "JSX",
    colorHint: "#f7df1e",
  },
  json: {
    key: "json",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M284.096 382.755c126.41-96.067 39.609-294.896-69.513-277.86C9.315 136.94 60.658 422.223 129.047 475.258C22.042 422.39-22.426 291.217 10.82 179.74C37.161 91.427 119.287 3.962 243.708.61c196.367-17.91 254.873 364.165 40.388 382.145M219.247 130.2c-112.3 5.728-136.522 119.158-127.29 201.114c13.467 119.577 89.242 197.216 207.553 177.684c181.777-30.01 331.904-318.726 76.441-480.015c94.11 109.624 94.983 331.713-59.624 371.13c-142.981 43.102-208.076-187.725-97.08-269.913"/></svg>`,
    description: "JSON",
    colorHint: "#cb3837",
  },
  css: {
    key: "css",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from coolicons by Kryston Schwarze - https://creativecommons.org/licenses/by/4.0/ --><path fill="currentColor" d="m11.983 20.989l-6.37-1.813l-1.42-16.033h15.615l-1.42 16.034l-6.4 1.812h-.004Zm-4.261-7.637l.216 2.867L12 17.483l3.99-1.14l.906-9.923h-9.8l.158 1.949h7.529l-.186 2.024H9.66l.178 1.912h4.6l-.272 2.62l-2.164.6l-2.2-.6l-.14-1.57h-1.94v-.003Z"/></svg>`,
    description: "CSS",
    colorHint: "#563d7c",
  },
  html: {
    key: "html",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Remix Icon by Remix Design - https://github.com/Remix-Design/RemixIcon/blob/master/License --><path fill="currentColor" d="m12 18.178l4.62-1.256l.623-6.778H9.026L8.822 7.89h8.626l.227-2.211H6.325l.636 6.678h7.82l-.261 2.866l-2.52.667l-2.52-.667l-.158-1.844h-2.27l.329 3.544zM3 2h18l-1.623 18L12 22l-7.377-2z"/></svg>`,
    description: "HTML",
    colorHint: "#e34c26",
  },
  md: {
    key: "md",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Material Symbols by Google - https://github.com/google/material-design-icons/blob/master/LICENSE --><path fill="currentColor" d="m16 15l3-3l-1.05-1.075l-1.2 1.2V9h-1.5v3.125l-1.2-1.2L13 12zM4 20q-.825 0-1.412-.587T2 18V6q0-.825.588-1.412T4 4h16q.825 0 1.413.588T22 6v12q0 .825-.587 1.413T20 20zm1.5-5H7v-4.5h1v3h1.5v-3h1V15H12v-5q0-.425-.288-.712T11 9H6.5q-.425 0-.712.288T5.5 10z"/></svg>`,
    description: "Markdown",
    colorHint: "#4f4f4f",
  },

  // Systems & compiled
  rs: {
    key: "rs",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 32 32"><!-- Icon from CoreUI Brands by creativeLabs Łukasz Holeczek - https://creativecommons.org/publicdomain/zero/1.0/ --><path fill="currentColor" d="m31.584 15.615l-1.329-.823a7 7 0 0 0-.036-.385l1.141-1.063a.463.463 0 0 0-.152-.765l-1.459-.543a9 9 0 0 0-.113-.38l.905-1.26a.454.454 0 0 0-.296-.719l-1.537-.251q-.087-.172-.181-.344l.645-1.416a.45.45 0 0 0-.036-.443a.46.46 0 0 0-.396-.203l-1.563.057a7 7 0 0 0-.245-.303l.36-1.516a.46.46 0 0 0-.552-.552l-1.516.36a6 6 0 0 0-.303-.245l.057-1.563a.455.455 0 0 0-.645-.432l-1.416.645c-.115-.061-.229-.124-.349-.181l-.251-1.537a.458.458 0 0 0-.719-.296l-1.26.905q-.188-.06-.375-.113L19.415.796a.458.458 0 0 0-.76-.157l-1.063 1.147a7 7 0 0 0-.391-.041L16.384.421a.454.454 0 0 0-.776 0l-.823 1.324q-.197.015-.385.041L13.337.639a.46.46 0 0 0-.765.157l-.543 1.453c-.129.036-.249.077-.38.113l-1.26-.905a.458.458 0 0 0-.719.296L9.419 3.29c-.115.057-.228.12-.343.181l-1.417-.645a.455.455 0 0 0-.645.432l.052 1.563q-.15.119-.297.245l-1.52-.36a.458.458 0 0 0-.548.552l.355 1.516c-.084.099-.161.197-.245.303L3.254 7.02a.456.456 0 0 0-.432.645l.647 1.416l-.188.344l-1.536.251a.46.46 0 0 0-.297.719l.912 1.26a9 9 0 0 0-.115.38l-1.459.543a.46.46 0 0 0-.151.765l1.14 1.063a7 7 0 0 0-.036.385l-1.328.823a.45.45 0 0 0 0 .771l1.328.823q.013.198.036.385l-1.14 1.068a.454.454 0 0 0 .151.76l1.459.548q.052.186.115.375l-.912 1.26a.458.458 0 0 0 .301.719l1.537.251q.087.172.183.344l-.647 1.416a.456.456 0 0 0 .432.645l1.557-.052c.084.1.161.199.251.297l-.36 1.521a.456.456 0 0 0 .548.547l1.52-.355c.099.084.199.161.303.245l-.057 1.557a.453.453 0 0 0 .645.432l1.417-.645q.172.092.343.187l.256 1.537a.455.455 0 0 0 .713.296l1.265-.911q.188.06.375.113l.548 1.459a.452.452 0 0 0 .76.152l1.063-1.141q.192.026.385.041l.823 1.328a.457.457 0 0 0 .776 0l.823-1.328c.131-.009.256-.025.385-.041l1.063 1.141a.456.456 0 0 0 .76-.152l.548-1.459q.186-.053.375-.113l1.26.911a.456.456 0 0 0 .719-.296l.251-1.537q.179-.094.349-.187l1.416.645a.457.457 0 0 0 .645-.432l-.057-1.557c.105-.084.204-.161.297-.245l1.521.355a.46.46 0 0 0 .552-.547l-.36-1.521q.127-.148.245-.297l1.563.052a.455.455 0 0 0 .432-.645l-.645-1.416q.093-.172.181-.344l1.537-.251a.458.458 0 0 0 .296-.719l-.905-1.26l.113-.375l1.453-.548a.455.455 0 0 0 .157-.76l-1.141-1.068c.011-.124.027-.255.036-.385l1.329-.823a.45.45 0 0 0 0-.771zm-8.881 11c-1.224-.26-.833-2.099.396-1.839a.94.94 0 0 1-.396 1.839m-.448-3.047a.853.853 0 0 0-1.015.661l-.475 2.192A11.5 11.5 0 0 1 16 27.453a11.5 11.5 0 0 1-4.864-1.073l-.475-2.197a.85.85 0 0 0-1.016-.656l-1.937.411a11 11 0 0 1-1.005-1.183h9.443c.105 0 .177-.015.177-.115v-3.337c0-.1-.072-.115-.177-.115h-2.765v-2.12h2.989c.271 0 1.459.077 1.833 1.593c.12.464.381 1.979.557 2.464c.177.547.901 1.629 1.672 1.629h4.704a1 1 0 0 0 .171-.015a11 11 0 0 1-1.067 1.255zM9.197 26.573a.95.95 0 0 1-1.119-.724a.933.933 0 0 1 .724-1.115a.94.94 0 0 1 .395 1.839M5.615 12.047a.945.945 0 0 1-.464 1.271a.95.95 0 0 1-1.255-.505c-.459-1.124 1.192-1.859 1.719-.765zm-1.099 2.609l2.02-.896c.433-.192.625-.697.433-1.129l-.417-.943h1.641v7.38H4.89a11.6 11.6 0 0 1-.375-4.412zm8.869-.713v-2.177h3.896c.203 0 1.421.235 1.421 1.147c0 .76-.937 1.031-1.703 1.031zm14.167 1.958q0 .431-.031.853h-1.188c-.12 0-.167.079-.167.199v.541c0 1.281-.719 1.557-1.355 1.631c-.604.068-1.271-.251-1.353-.62c-.355-2-.948-2.427-1.885-3.167c1.161-.74 2.369-1.823 2.369-3.281c0-1.573-1.079-2.563-1.812-3.047c-1.032-.683-2.172-.817-2.48-.817H7.395a11.55 11.55 0 0 1 6.459-3.652l1.448 1.521a.854.854 0 0 0 1.208.025l1.615-1.547a11.55 11.55 0 0 1 7.911 5.631l-1.109 2.5a.863.863 0 0 0 .437 1.131l2.131.947c.036.381.052.761.052 1.152zM15.303 3.255c.905-.864 2.203.495 1.296 1.36c-.901.781-2.115-.495-1.296-1.36m10.984 8.838a.937.937 0 0 1 1.235-.479c.771.339.744 1.437-.037 1.74c-.787.301-1.541-.495-1.197-1.261z"/></svg>`,
    description: "Rust",
    colorHint: "#dea584",
  },
  go: {
    key: "go",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="m37.435 218.2l7.266-9.271l92.304-.044l-5.943 9.27zm83.718 22.805l3.061-9.27l-116.949.08L0 241.085zm-3.361 22.887l.884-9.27l-53.197.043l-5.163 9.27zm339.301 72.6c-51.18 28.193-121.104 16.253-139.798-44.876c-11.048 21.741-29.087 39.56-50.584 49.721c-50.933 24.075-120.085 3.08-128.979-62.896c-6.513-63.238 44.669-102.314 74.925-112.18c48.085-14.979 95.508 2.622 111.907 46.427l-51.65 13.538c-5.488-15.271-35.313-26.268-58.239-9.693c-36.057 22.247-35.391 78.232 8.173 86.545c22.618 2.9 37.81-8.276 49.2-25.466l-56.087.12l18.021-38.86h84.057c36.406-102.707 178.391-98.21 192.821-5.92c5.129 32.802-6.069 77.264-53.767 103.54M410.447 209.42c-38.861 8.362-48.547 42.69-42.548 64.684c6.383 23.406 31.11 38.754 62.482 24.076c50.28-23.523 37.1-101.032-19.934-88.76"/></svg>`,
    description: "Go",
    colorHint: "#00acd7",
  },
  c: {
    key: "c",
    svg: PLACEHOLDER_SVG("c"),
    description: "C",
    colorHint: "#555",
  },
  cpp: {
    key: "cpp",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE_ALT}" height="${ICON_SIZE}" viewBox="0 0 455 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M435.28 113.284L247.19 4.692c-10.835-6.256-28.567-6.256-39.402 0L19.701 113.284C8.865 119.54 0 134.896 0 147.408v217.184c.843 14.572 8.286 26.64 19.7 34.124l188.09 108.591c10.834 6.257 28.566 6.257 39.401 0L435.28 398.716c12.248-8.146 19.22-20.408 19.701-34.124V147.408c-.44-13.165-8.323-26.443-19.7-34.124M75.83 256.001c-.891-148.09 197.46-213.48 282.996-75.878l-65.634 37.98c-39.622-65.533-138.001-42.1-141.532 37.898c1.06 73.729 97.754 107.978 141.536 37.892l65.635 37.98c-75.383 130.862-279.2 83.78-283-75.872m303.32 8.424h-16.852v16.852H345.45v-16.852h-16.852v-16.85h16.852v-16.85h16.85v16.85h16.851zm63.192 0H425.49v16.852h-16.85v-16.852h-16.85v-16.85h16.852v-16.85h16.849v16.85h16.852z"/></svg>`,
    description: "C++",
    colorHint: "#004482",
  },
  h: {
    key: "h",
    svg: PLACEHOLDER_SVG("h"),
    description: "C/C++ Header",
    colorHint: "#6a6a6a",
  },
  hpp: {
    key: "hpp",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE_ALT}" height="${ICON_SIZE}" viewBox="0 0 455 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M435.28 113.284L247.19 4.692c-10.835-6.256-28.567-6.256-39.402 0L19.701 113.284C8.865 119.54 0 134.896 0 147.408v217.184c.843 14.572 8.286 26.64 19.7 34.124l188.09 108.591c10.834 6.257 28.566 6.257 39.401 0L435.28 398.716c12.248-8.146 19.22-20.408 19.701-34.124V147.408c-.44-13.165-8.323-26.443-19.7-34.124M75.83 256.001c-.891-148.09 197.46-213.48 282.996-75.878l-65.634 37.98c-39.622-65.533-138.001-42.1-141.532 37.898c1.06 73.729 97.754 107.978 141.536 37.892l65.635 37.98c-75.383 130.862-279.2 83.78-283-75.872m303.32 8.424h-16.852v16.852H345.45v-16.852h-16.852v-16.85h16.852v-16.85h16.85v16.85h16.851zm63.192 0H425.49v16.852h-16.85v-16.852h-16.85v-16.85h16.852v-16.85h16.849v16.85h16.852z"/></svg>`,
    description: "C++ Header",
    colorHint: "#6a6a6a",
  },
  swift: {
    key: "swift",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Material Design Icons by Pictogrammers - https://github.com/Templarian/MaterialDesign/blob/master/LICENSE --><path fill="currentColor" d="M17.09 19.72c-2.36 1.36-5.59 1.5-8.86.1A13.8 13.8 0 0 1 2 14.5c.67.55 1.46 1 2.3 1.4c3.37 1.57 6.73 1.46 9.1 0c-3.37-2.59-6.24-5.96-8.37-8.71c-.45-.45-.78-1.01-1.12-1.51c8.28 6.05 7.92 7.59 2.41-1.01c4.89 4.94 9.43 7.74 9.43 7.74c.16.09.25.16.36.22c.1-.25.19-.51.26-.78c.79-2.85-.11-6.12-2.08-8.81c4.55 2.75 7.25 7.91 6.12 12.24c-.03.11-.06.22-.05.39c2.24 2.83 1.64 5.78 1.35 5.22c-1.21-2.39-3.48-1.65-4.62-1.17"/></svg>`,
    description: "Swift",
    colorHint: "#ffac45",
  },
  zig: {
    key: "zig",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M270.276 194.86H146.401v-61.905h179.17L447.701 88L242.755 318.67H366.92v61.91H187.75L65.615 424zM0 132.957l122.565-.005v61.905H55.127v123.815h82.691l-55.127 61.905H0zm456.868 61.904h-82.69l55.131-61.905H512v247.62l-122.565.004v-61.905h67.433z"/></svg>`,
    description: "Zig",
    colorHint: "#f7a41d",
  },

  // Other languages
  py: {
    key: "py",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Material Design Icons by Pictogrammers - https://github.com/Templarian/MaterialDesign/blob/master/LICENSE --><path fill="currentColor" d="M19.14 7.5A2.86 2.86 0 0 1 22 10.36v3.78A2.86 2.86 0 0 1 19.14 17H12c0 .39.32.96.71.96H17v1.68a2.86 2.86 0 0 1-2.86 2.86H9.86A2.86 2.86 0 0 1 7 19.64v-3.75a2.85 2.85 0 0 1 2.86-2.85h5.25a2.85 2.85 0 0 0 2.85-2.86V7.5zm-4.28 11.79c-.4 0-.72.3-.72.89s.32.71.72.71a.71.71 0 0 0 .71-.71c0-.59-.32-.89-.71-.89m-10-1.79A2.86 2.86 0 0 1 2 14.64v-3.78A2.86 2.86 0 0 1 4.86 8H12c0-.39-.32-.96-.71-.96H7V5.36A2.86 2.86 0 0 1 9.86 2.5h4.28A2.86 2.86 0 0 1 17 5.36v3.75a2.85 2.85 0 0 1-2.86 2.85H8.89a2.85 2.85 0 0 0-2.85 2.86v2.68zM9.14 5.71c.4 0 .72-.3.72-.89s-.32-.71-.72-.71c-.39 0-.71.12-.71.71s.32.89.71.89"/></svg>`,
    description: "Python",
    colorHint: "#3670a0",
  },
  java: {
    key: "java",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Remix Icon by Remix Design - https://github.com/Remix-Design/RemixIcon/blob/master/License --><path fill="currentColor" d="m15.638 4.566l.056.032c-.758.4-2.924 1.69-2.924 3.332c0 .554.317 1.088.614 1.59c.262.442.509.858.509 1.238c0 .957-.933 1.7-1.46 2.042l-.1-.058c.199-.243.444-.65.444-1.084c0-.598-.307-1.076-.618-1.561c-.322-.501-.648-1.01-.648-1.67c0-2.292 3.115-3.522 4.127-3.861m-4.095 1.212c1.253-1.12 2.622-2.344 2.622-4.185c0-.833-.341-1.365-.51-1.578L13.6.046c.04.166.1.472.1.872c0 1.676-1.422 2.85-2.798 3.988C9.611 5.974 8.36 7.008 8.36 8.392c0 1.985 1.958 3.206 2.785 3.722l.063.04l.05-.03q-.067-.074-.142-.152c-.636-.677-1.602-1.704-1.602-3.275c0-1.103.974-1.974 2.03-2.919m-.452 9.908c1.764 0 2.998-.253 3.546-.408l.832.48c-.793.403-2.551.71-4.382.71c-2.153 0-4.507-.462-4.514-1.078c-.005-.34.765-.566 1.595-.712l.05.029s-.281.101-.278.333c.004.35 1.42.646 3.15.646m-3.529 2.171c0-.407.839-.6 1.223-.677l.05.03c-.066.049-.102.116-.102.173c0 .267.93.511 2.356.511c1.278 0 1.988-.157 2.41-.258l.99.573c-.045.032-1.02.645-3.402.645c-1.731 0-3.525-.432-3.525-.997m8.529-1.728c1.18-.673 2.361-1.469 2.428-2.747c.044-.839-.727-1.454-1.57-1.29l.045-.112v-.002c.212-.064.474-.116.767-.116c.943 0 1.666.565 1.758 1.356c.186 1.586-2.062 2.618-3.321 2.973zm1.975 2.988c.01 1.09-3.698 1.738-7.012 1.767c-2.861.025-7.474-.515-7.484-1.605c-.006-.753 2-1.274 3.09-1.424l.115.065s-1.625.377-1.62 1.062c.006.683 3.425 1.274 5.894 1.253c3.825-.034 6.414-.657 6.72-1.502l.054-.031c.112.082.24.217.243.415M6.43 21.337a26 26 0 0 0 4.279.325c6.208-.054 7.96-1.58 8.23-1.912l.047.028c-.064 1.208-3.347 2.212-7.396 2.247c-2.061.018-3.937-.22-5.285-.615zm2.602-9.283c-1.079.083-3.396.426-3.396 1.036c0 .462 2.124 1.113 5.452 1.113c2.994 0 4.884-.565 5.325-.78l-.643-.375c-.46.125-2.169.506-4.682.506c-1.48 0-4.03-.273-4.03-.69c0-.374 1.591-.662 2.048-.745l.029-.005z"/></svg>`,
    description: "Java",
    colorHint: "#e76f00",
  },
  kt: {
    key: "kt",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Material Design Icons by Pictogrammers - https://github.com/Templarian/MaterialDesign/blob/master/LICENSE --><path fill="currentColor" d="M2 2h20L12 12l10 10H2Z"/></svg>`,
    description: "Kotlin",
    colorHint: "#a97bff",
  },
  dart: {
    key: "dart",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 128 128"><!-- Icon from Devicon Plain by konpa - https://github.com/devicons/devicon/blob/master/LICENSE --><g fill="currentColor"><path d="m86.6 25l3 .1c1.1.1 2.2.3 3.4.5l-2.5-7.4L75.7 3.5c-3.4-3.4-8-4.4-10.4-2.3L29.2 25.1zm6.1 3.6c-1.2-.2-2.3-.4-3.3-.5l-2.9-.1l-56 .1l78.6 78.6l6.1-13.8zM28.9 92.2l64.3 22.7l13.8-6.1l-78.6-78.6v56.1l.1 2.7c0 .9.1 2 .4 3.2"/><path d="M106.9 34.3c-2.6-2.6-7-5.1-11.3-6.5L118.4 93l-6.9 15.7l15.8-5.2V54.8zm-13.5 83.8l-65-22.9c1.4 4.3 3.8 8.7 6.5 11.4l21.3 21.2l47.6.1l5.3-16.7zm-67.9-29l-.1-2.7V28.9L1.7 65.1C-.4 67.3.7 72 4 75.5l14.7 14.8l7.3 2.6c-.3-1.3-.5-2.5-.5-3.8"/></g></svg>`,
    description: "Dart",
    colorHint: "#055393",
  },
  php: {
    key: "php",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Remix Icon by Remix Design - https://github.com/Remix-Design/RemixIcon/blob/master/License --><path fill="currentColor" d="M4.931 9.46H3.594l-.73 3.751h1.187q1.181 0 1.76-.445q.577-.445.779-1.486q.194-.999-.176-1.41t-1.483-.41m2.45 4.153c-.369.354-.814.62-1.298.78q-.715.232-1.82.233H2.588l-.463 2.382H.174l1.742-8.962H5.67q1.694 0 2.471.89q.778.888.468 2.482a4 4 0 0 1-.432 1.2a4 4 0 0 1-.795.995m5.701 1.013l.77-3.965q.132-.677-.097-.922q-.227-.247-.973-.247h-1.547l-.997 5.136H8.3l1.742-8.964h1.937l-.463 2.383h1.725q1.63 0 2.247.568t.373 1.84l-.81 4.171zm10.763-3.208c-.078.42-.224.825-.432 1.2a4 4 0 0 1-.795.995c-.37.353-.814.62-1.299.78q-.715.232-1.822.233h-1.671l-.464 2.383H15.41l1.743-8.962h3.752q1.694 0 2.471.89q.778.886.469 2.48M20.169 9.46h-1.336l-.731 3.751h1.187q1.183 0 1.76-.445t.78-1.486q.194-.999-.177-1.41t-1.483-.41"/></svg>`,
    description: "PHP",
    colorHint: "#777bb3",
  },
  lua: {
    key: "lua",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" fill-rule="evenodd" d="M255.916 63.517c106.273 0 192.425 86.151 192.425 192.425s-86.152 192.425-192.425 192.425S63.49 362.215 63.49 255.942S149.642 63.517 255.916 63.517m79.708 56.364c31.126 0 56.359 25.232 56.359 56.358s-25.233 56.359-56.359 56.359s-56.358-25.233-56.358-56.359s25.232-56.358 56.358-56.358M97.317 74.199l-10.02-11.501a256.5 256.5 0 0 1 26.783-20.39l8.422 12.718a241 241 0 0 0-25.185 19.173M43.06 398.96a256 256 0 0 0 20.539 26.671l11.445-10.084a241 241 0 0 1-19.312-25.079zM179.288 27.28l-4.849-14.463a257 257 0 0 0-31.148 12.775l6.701 13.703a242 242 0 0 1 29.296-12.015M74.886 96.518L63.436 86.44a256 256 0 0 0-20.526 26.68l12.677 8.484a241 241 0 0 1 19.298-25.087m-35.142 52.497l-13.678-6.753a259 259 0 0 0-12.896 31.09l14.442 4.911a243 243 0 0 1 12.132-29.248M143.5 486.422a259 259 0 0 0 31.158 12.725l4.834-14.468a243 243 0 0 1-29.315-11.972zM13.217 338.778a259 259 0 0 0 12.968 31.062l13.666-6.778a244 244 0 0 1-12.204-29.228zm483.212-101.422l15.21-1.17a255 255 0 0 0-4.796-33.33l-14.922 3.167a240 240 0 0 1 4.508 31.333M19.392 208.844l-14.958-2.989a258 258 0 0 0-4.423 33.37l15.22 1.008a243 243 0 0 1 4.16-31.389m-4.17 63.047L0 272.89a255.5 255.5 0 0 0 4.426 33.382l14.956-3a240 240 0 0 1-4.16-31.382m72.25 177.47a256 256 0 0 0 26.794 20.376l8.415-12.724a241 241 0 0 1-25.194-19.16zm304.068 6.034l8.561 12.625a256 256 0 0 0 26.558-20.686l-10.148-11.39a241 241 0 0 1-24.971 19.451M241.315 15.226L240.393 0a257 257 0 0 0-33.398 4.237l2.91 14.974a242 242 0 0 1 31.41-3.985m197.379 398.152l11.56 9.951a256 256 0 0 0 20.23-26.906l-12.77-8.344a241 241 0 0 1-19.02 25.3m34.562-52.882l13.752 6.603a259 259 0 0 0 12.554-31.228l-14.494-4.755a243 243 0 0 1-11.812 29.38m23.513-91.48a242.5 242.5 0 0 1-3.815 31.431l14.99 2.826A258 258 0 0 0 512 269.856zm.923-98.565a260 260 0 0 0-13.305-30.918l-13.592 6.926a244.5 244.5 0 0 1 12.522 29.095zM207.21 507.706a258 258 0 0 0 33.394 4.24l.925-15.226a242 242 0 0 1-31.41-3.989zM307.406 4.75a258 258 0 0 0-33.35-4.577l-1.078 15.216a242 242 0 0 1 31.367 4.305zm-34.218 491.804l1.084 15.216a255.5 255.5 0 0 0 33.357-4.61l-3.083-14.94a240 240 0 0 1-31.358 4.334m97.694-469.812a257 257 0 0 0-31.013-13.086l-4.993 14.414a242 242 0 0 1 29.166 12.306zM335.06 483.786l5.025 14.403a260 260 0 0 0 30.99-13.137l-6.852-13.629a244 244 0 0 1-29.163 12.363m113.286-363.905c31.13 0 56.364-25.235 56.364-56.364S479.475 7.153 448.346 7.153s-56.364 25.235-56.364 56.364s25.235 56.364 56.364 56.364" clip-rule="evenodd"/></svg>`,
    description: "Lua",
    colorHint: "#000080",
  },
  rb: {
    key: "rb",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Material Design Icons by Pictogrammers - https://github.com/Templarian/MaterialDesign/blob/master/LICENSE --><path fill="currentColor" d="M18.8 2.07c2.52.43 3.24 2.16 3.2 3.97V6l-1.14 14.93l-14.78 1.01h.01c-1.23-.05-3.96-.17-4.09-3.99l1.37-2.5l2.77 6.46l2.36-7.62l-.05.01l.02-.02l7.71 2.46l-1.99-7.78l7.35-.46l-5.79-4.74l3.05-1.7zM2 17.91v.02zM6.28 6.23c2.96-2.95 6.79-4.69 8.26-3.2c1.46 1.47-.08 5.09-3.04 8.03c-3 2.94-6.77 4.78-8.24 3.3c-1.47-1.49.04-5.19 3.01-8.13z"/></svg>`,
    description: "Ruby",
    colorHint: "#cc342d",
  },
  hs: {
    key: "hs",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 24 24"><!-- Icon from Material Design Icons by Pictogrammers - https://github.com/Templarian/MaterialDesign/blob/master/LICENSE --><path fill="currentColor" d="m2.08 19l4.67-7l-4.67-7h3.5l4.67 7l-4.67 7zm4.67 0l4.67-7l-4.67-7h3.5l9.34 14h-3.5l-2.92-4.37L10.25 19zm11.28-4.08l-1.53-2.34h5.42v2.34zm-2.33-3.5l-1.56-2.34h7.78v2.34z"/></svg>`,
    description: "Haskell",
    colorHint: "#5e5086",
  },
  ml: {
    key: "ml",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M0 223.6V97.4c0-35.6 28.9-64.5 64.5-64.5h383c35.6 0 64.5 28.9 64.5 64.5V479H299.5s-14.1-37-16.8-55.2c-.7-4.7.7-9.4-.8-13.8c-1.2-3.8-4-4.2-5.8-7.3c-4.8-8.2-9.8-18.8-10.2-28.8c-.4-9-3.7-17.2-4.2-26.1c-.2-4.3.3-8.8.1-13c-.1-2.1-.3-6.9-1.3-9.9l1.1-2.8c-.5-1 9.6-.6 12.6-.6c5.1.1 9.9.3 15 .6c10.4.5 19.8.4 29.9-1.2c22.5-3.5 32.9-12.7 38.2-16.5c20.7-14.9 30.1-39.3 30.1-39.3c3.4-7.6 3.4-21.2 10.7-27.3c8.6-7.2 23.1-6.7 33-11.1c5.8-2.6 10-4 15.9-2.7c4.4.9 12.3 6 14.1-1.1c-1.5-.9-2-2.7-2.8-3.6c8.2-.8.2-19.7-3.1-23.5c-5-5.8-13.3-8.5-22.1-10.8c-10.5-2.8-20-6-29.9-4.1c-17.3 3.4-16-6.5-26.2-6.5c-12.2 0-33.9.6-37.7 12.5c-1.7 5.5-3.6 5.8-6.6 10c-2.6 3.6.5 6.9-.9 11c-1.3 4.3-3.3 19.3-5.4 24.6c-3.5 8.9-7.6 20-15.3 20c-10.7 1.3-19.2 1.7-27.9-1.5c-5.2-1.9-14-4.9-18.4-6.7c-20.1-8.4-23.4-17.7-23.4-17.7c-2.2-3.6-23.4-47.8-26.5-55.4c-4-9.9-9.6-17.2-13.6-20.9c-6.2-5.6-12-14.2-24.7-11.7c-2.3.4-10.5.8-16.8 6.1c-4.3 3.6-5.6 11-9.6 17.3c-2.3 3.6-6.3 14-10 22.6c-2.6 6-3.8 10.5-6.5 12.7c-2.2 1.7-7.5 6-14.4-1c-2.6-2.7-9-13.9-13.7-22c-13.7-23.8-19.1-34.5-34.8-34.6c-20-.1-21.5 11.2-30.3 27.5c0-.1-8.8 33.1-40.5 56.4m0 146.1V479h99.2s8-30.9 9.8-37.2c1.5-4.8 3.4-8.7 4.2-13.6c.7-4.7-.1-9.2-.5-13.4c-1-10.7 7.8-14.5 12-23.7c3.8-8.3 6-17.7 9.1-26.2c3-8.1 7.7-19.6 15.8-23.7c-1-1.1-16.8-1.6-21-2.1c-13-1.4-31.5-5.3-39.9-7.3c-5.1-1.2-22.1-7.3-25.8-9c-8.7-4-14.4-14.8-21.2-13.7c-4.3.7-8.5 2.2-11.2 6.5c-2.2 3.6-2.9 9.7-4.5 13.8c-1.8 4.8-4.8 9.2-7.5 13.7c-4.7 8.3-15.4 17.3-18.5 26.6m275.9 92.6c-1.9-4-4.3-11.8-5.9-15.3c-1.5-3.2-6.1-11.6-8.4-14.3c-5-5.9-6.2-6.3-7.7-13.7c-2.6-12.9-9.3-36.3-17.3-52.4c-4.1-8.3-11-15.3-17.3-21.3c-5.5-5.3-17.8-14.2-20-13.8c-20.2 4-26.4 23.8-35.9 39.5c-5.3 8.7-10.8 16-15 25.3c-3.8 8.5-3.5 17.9-10 25.2c-6.7 7.5-11.1 15.4-14.4 25.1c-.6 1.8-6.3 32.4-6.3 32.4h164.7l-.3-.7c-2.2-6.3-5.1-13.6-6.2-16"/></svg>`,
    description: "OCaml",
    colorHint: "#ef7a08",
  },
  mli: {
    key: "mli",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M0 223.6V97.4c0-35.6 28.9-64.5 64.5-64.5h383c35.6 0 64.5 28.9 64.5 64.5V479H299.5s-14.1-37-16.8-55.2c-.7-4.7.7-9.4-.8-13.8c-1.2-3.8-4-4.2-5.8-7.3c-4.8-8.2-9.8-18.8-10.2-28.8c-.4-9-3.7-17.2-4.2-26.1c-.2-4.3.3-8.8.1-13c-.1-2.1-.3-6.9-1.3-9.9l1.1-2.8c-.5-1 9.6-.6 12.6-.6c5.1.1 9.9.3 15 .6c10.4.5 19.8.4 29.9-1.2c22.5-3.5 32.9-12.7 38.2-16.5c20.7-14.9 30.1-39.3 30.1-39.3c3.4-7.6 3.4-21.2 10.7-27.3c8.6-7.2 23.1-6.7 33-11.1c5.8-2.6 10-4 15.9-2.7c4.4.9 12.3 6 14.1-1.1c-1.5-.9-2-2.7-2.8-3.6c8.2-.8.2-19.7-3.1-23.5c-5-5.8-13.3-8.5-22.1-10.8c-10.5-2.8-20-6-29.9-4.1c-17.3 3.4-16-6.5-26.2-6.5c-12.2 0-33.9.6-37.7 12.5c-1.7 5.5-3.6 5.8-6.6 10c-2.6 3.6.5 6.9-.9 11c-1.3 4.3-3.3 19.3-5.4 24.6c-3.5 8.9-7.6 20-15.3 20c-10.7 1.3-19.2 1.7-27.9-1.5c-5.2-1.9-14-4.9-18.4-6.7c-20.1-8.4-23.4-17.7-23.4-17.7c-2.2-3.6-23.4-47.8-26.5-55.4c-4-9.9-9.6-17.2-13.6-20.9c-6.2-5.6-12-14.2-24.7-11.7c-2.3.4-10.5.8-16.8 6.1c-4.3 3.6-5.6 11-9.6 17.3c-2.3 3.6-6.3 14-10 22.6c-2.6 6-3.8 10.5-6.5 12.7c-2.2 1.7-7.5 6-14.4-1c-2.6-2.7-9-13.9-13.7-22c-13.7-23.8-19.1-34.5-34.8-34.6c-20-.1-21.5 11.2-30.3 27.5c0-.1-8.8 33.1-40.5 56.4m0 146.1V479h99.2s8-30.9 9.8-37.2c1.5-4.8 3.4-8.7 4.2-13.6c.7-4.7-.1-9.2-.5-13.4c-1-10.7 7.8-14.5 12-23.7c3.8-8.3 6-17.7 9.1-26.2c3-8.1 7.7-19.6 15.8-23.7c-1-1.1-16.8-1.6-21-2.1c-13-1.4-31.5-5.3-39.9-7.3c-5.1-1.2-22.1-7.3-25.8-9c-8.7-4-14.4-14.8-21.2-13.7c-4.3.7-8.5 2.2-11.2 6.5c-2.2 3.6-2.9 9.7-4.5 13.8c-1.8 4.8-4.8 9.2-7.5 13.7c-4.7 8.3-15.4 17.3-18.5 26.6m275.9 92.6c-1.9-4-4.3-11.8-5.9-15.3c-1.5-3.2-6.1-11.6-8.4-14.3c-5-5.9-6.2-6.3-7.7-13.7c-2.6-12.9-9.3-36.3-17.3-52.4c-4.1-8.3-11-15.3-17.3-21.3c-5.5-5.3-17.8-14.2-20-13.8c-20.2 4-26.4 23.8-35.9 39.5c-5.3 8.7-10.8 16-15 25.3c-3.8 8.5-3.5 17.9-10 25.2c-6.7 7.5-11.1 15.4-14.4 25.1c-.6 1.8-6.3 32.4-6.3 32.4h164.7l-.3-.7c-2.2-6.3-5.1-13.6-6.2-16"/></svg>`,
    description: "OCaml Interface",
    colorHint: "#ef7a08",
  },

  // Shell / scripts
  sh: {
    key: "sh",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 16 16"><!-- Icon from Codicons by Microsoft Corporation - https://github.com/microsoft/vscode-codicons/blob/main/LICENSE --><path fill="currentColor" d="M13.655 3.56L8.918.75a1.79 1.79 0 0 0-1.82 0L2.363 3.56a1.89 1.89 0 0 0-.921 1.628v5.624a1.89 1.89 0 0 0 .913 1.627l4.736 2.812a1.79 1.79 0 0 0 1.82 0l4.736-2.812a1.89 1.89 0 0 0 .913-1.627V5.188a1.89 1.89 0 0 0-.904-1.627zm-3.669 8.781v.404a.15.15 0 0 1-.07.124l-.239.137c-.038.02-.07 0-.07-.053v-.396a.78.78 0 0 1-.545.053a.07.07 0 0 1-.027-.09l.086-.365a.15.15 0 0 1 .071-.096a.05.05 0 0 1 .038 0a.66.66 0 0 0 .497-.063a.66.66 0 0 0 .37-.567c0-.206-.112-.292-.384-.293c-.344 0-.661-.066-.67-.574A1.47 1.47 0 0 1 9.6 9.437V9.03a.15.15 0 0 1 .07-.126l.231-.147c.038-.02.07 0 .07.054v.409a.75.75 0 0 1 .453-.055a.073.073 0 0 1 .03.095l-.081.362a.16.16 0 0 1-.065.09a.06.06 0 0 1-.035 0a.6.6 0 0 0-.436.072a.55.55 0 0 0-.331.486c0 .185.098.242.425.248c.438 0 .627.199.632.639a1.57 1.57 0 0 1-.576 1.185zm2.481-.68a.1.1 0 0 1-.036.092l-1.198.727a.034.034 0 0 1-.04.003a.04.04 0 0 1-.016-.037v-.31a.09.09 0 0 1 .055-.076l1.179-.706a.035.035 0 0 1 .056.035v.273zm.827-6.914L8.812 7.515c-.559.331-.97.693-.97 1.367v5.52c0 .404.165.662.413.741a1.5 1.5 0 0 1-.248.025c-.264 0-.522-.072-.748-.207L2.522 12.15a1.56 1.56 0 0 1-.75-1.338V5.188a1.56 1.56 0 0 1 .75-1.34l4.738-2.81a1.46 1.46 0 0 1 1.489 0l4.736 2.812a1.55 1.55 0 0 1 .728 1.083c-.154-.334-.508-.427-.92-.185h.002z"/></svg>`,
    description: "Shell",
    colorHint: "#4eaa25",
  },
  bash: {
    key: "bash",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 16 16"><!-- Icon from Codicons by Microsoft Corporation - https://github.com/microsoft/vscode-codicons/blob/main/LICENSE --><path fill="currentColor" d="M13.655 3.56L8.918.75a1.79 1.79 0 0 0-1.82 0L2.363 3.56a1.89 1.89 0 0 0-.921 1.628v5.624a1.89 1.89 0 0 0 .913 1.627l4.736 2.812a1.79 1.79 0 0 0 1.82 0l4.736-2.812a1.89 1.89 0 0 0 .913-1.627V5.188a1.89 1.89 0 0 0-.904-1.627zm-3.669 8.781v.404a.15.15 0 0 1-.07.124l-.239.137c-.038.02-.07 0-.07-.053v-.396a.78.78 0 0 1-.545.053a.07.07 0 0 1-.027-.09l.086-.365a.15.15 0 0 1 .071-.096a.05.05 0 0 1 .038 0a.66.66 0 0 0 .497-.063a.66.66 0 0 0 .37-.567c0-.206-.112-.292-.384-.293c-.344 0-.661-.066-.67-.574A1.47 1.47 0 0 1 9.6 9.437V9.03a.15.15 0 0 1 .07-.126l.231-.147c.038-.02.07 0 .07.054v.409a.75.75 0 0 1 .453-.055a.073.073 0 0 1 .03.095l-.081.362a.16.16 0 0 1-.065.09a.06.06 0 0 1-.035 0a.6.6 0 0 0-.436.072a.55.55 0 0 0-.331.486c0 .185.098.242.425.248c.438 0 .627.199.632.639a1.57 1.57 0 0 1-.576 1.185zm2.481-.68a.1.1 0 0 1-.036.092l-1.198.727a.034.034 0 0 1-.04.003a.04.04 0 0 1-.016-.037v-.31a.09.09 0 0 1 .055-.076l1.179-.706a.035.035 0 0 1 .056.035v.273zm.827-6.914L8.812 7.515c-.559.331-.97.693-.97 1.367v5.52c0 .404.165.662.413.741a1.5 1.5 0 0 1-.248.025c-.264 0-.522-.072-.748-.207L2.522 12.15a1.56 1.56 0 0 1-.75-1.338V5.188a1.56 1.56 0 0 1 .75-1.34l4.738-2.81a1.46 1.46 0 0 1 1.489 0l4.736 2.812a1.55 1.55 0 0 1 .728 1.083c-.154-.334-.508-.427-.92-.185h.002z"/></svg>`,
    description: "Bash",
    colorHint: "#4eaa25",
  },
  ps1: {
    key: "ps1",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M404.858 448.69H20.423c-13.115 0-22.827-12.189-19.899-24.972l75.54-329.715C80.065 76.066 91.682 63.31 110.06 63.31h382.244c12.644 0 22.01 11.749 19.192 24.075L434.25 425.26c-3.134 13.709-15.33 23.43-29.392 23.43m-274.85-51.538l176.803-126.98c10.735-7.753 13.02-24.357 3.266-33.796L193.262 112.328c-19.247-20.44-52.1 10.498-32.853 30.937l99.144 105.286l-155.87 111.947c-24.91 17.89 1.415 54.544 26.325 36.654m216.15-43.913h-93.839c-27.033 0-27.033 40.83 0 40.83h93.839c26.63 0 26.63-40.83 0-40.83"/></svg>`,
    description: "PowerShell",
    colorHint: "#0273d4",
  },

  // Data / misc
  sql: {
    key: "sql",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 16 16"><!-- Icon from Gravity UI Icons by YANDEX LLC - https://github.com/gravity-ui/icons/blob/main/LICENSE --><g fill="none"><g clip-path="url(#SVGFd4lJeyK)"><path fill="currentColor" fill-rule="evenodd" d="M13.5 4.75a.75.75 0 0 0-1.5 0v6.5c0 .414.336.75.75.75h2.5a.75.75 0 0 0 0-1.5H13.5zM11 8c0 1.32-.257 2.314-.709 2.98l.99.99a.75.75 0 1 1-1.061 1.06l-1.132-1.132Q8.753 12 8.375 12C6.8 12 5.75 10.667 5.75 8S6.8 4 8.375 4S11 5.333 11 8m-1.957 2.15c.222-.322.457-.98.457-2.149c0-1.168-.235-1.826-.457-2.149c-.175-.254-.366-.35-.668-.35s-.493.096-.668.35c-.222.323-.457.981-.457 2.15c0 1.167.235 1.826.457 2.148c.175.254.366.351.668.351s.493-.097.668-.351M2.562 4.002c-.689 0-1.285.249-1.702.713C.453 5.17.277 5.76.277 6.352c0 .732.326 1.28.759 1.658c.41.358.908.561 1.305.684c.398.122.684.217.89.372c.147.113.27.266.27.625c0 .29-.109.468-.253.589a1.08 1.08 0 0 1-.686.222c-.202 0-.438-.093-.658-.293a1.4 1.4 0 0 1-.423-.684a.75.75 0 0 0-1.462.333c.127.557.453 1.079.878 1.463c.426.386 1.006.681 1.665.681c.578 0 1.177-.18 1.647-.57c.485-.405.791-1.006.791-1.74c0-.843-.352-1.433-.864-1.821c-.434-.329-.962-.49-1.306-.596l-.047-.015c-.31-.095-.579-.221-.76-.38a.65.65 0 0 1-.246-.528c0-.304.09-.512.2-.635c.099-.11.27-.215.585-.215c.322 0 .498.099.612.206c.13.123.233.313.3.56a.75.75 0 0 0 1.449-.386c-.113-.42-.325-.892-.719-1.263c-.409-.387-.961-.617-1.642-.617" clip-rule="evenodd"/></g><defs><clipPath id="SVGFd4lJeyK"><path fill="currentColor" d="M0 0h16v16H0z"/></clipPath></defs></g></svg>`,
    description: "SQL",
    colorHint: "#00758f",
  },
  yml: {
    key: "yml",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE * 0.89}" height="${ICON_SIZE}" viewBox="0 0 457 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M342.016 0H457L114.983 512H0l171.008-256L0 0h114.983L228.5 169.934z"/></svg>`,
    description: "YAML",
    colorHint: "#cb5832",
  },
  yaml: {
    key: "yaml",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE * 0.89}" height="${ICON_SIZE}" viewBox="0 0 457 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M342.016 0H457L114.983 512H0l171.008-256L0 0h114.983L228.5 169.934z"/></svg>`,
    description: "YAML",
    colorHint: "#cb5832",
  },
  toml: {
    key: "toml",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M512 512H397.47v-56.587h52.667V56.86H397.47V0H512zM375.885 107.743v59.344h-91.161v275.88h-64.107v-275.88h-91.586v-59.344zM0 0h113.963v56.587H61.295V455.14h52.668V512H0z"/></svg>`,
    description: "TOML",
    colorHint: "#9c4221",
  },

  // Config-ish
  env: {
    key: "env",
    svg: '<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 512 512"><!-- Icon from File Icons by John Gardner - https://github.com/file-icons/icons/blob/master/LICENSE.md --><path fill="currentColor" d="M0 0v512h512V0zm74.178 463.246H32.49V421.56h41.687zm37.772-.285v-136.64h100.743l.208 24.54h-74.18v29.024h68.636l.196 23.085H138.72v35.008h74.918l.212 24.983zm232.751 0h-28.785l-53.527-86.757v86.757h-25.765v-136.64h27.87l53.942 90.056V326.32h26.265zm92.732 0h-31.27l-48.02-136.64h31.304l33.411 95.734l31.754-95.734h31.557z"/></svg>',
    description: "Environment",
    colorHint: "#6d9e6d",
  },
};

export function getIconKey(fileName: string, isDir: boolean): string {
  if (isDir) return "folder";
  const lower = fileName.toLowerCase();
  if (ICONS[lower]) return lower;

  const lastDot = lower.lastIndexOf(".");
  if (lastDot === -1) return "file";
  const ext = lower.slice(lastDot + 1);
  if (ICONS[ext]) return ext;
  return "unknown";
}

export function getIconEntry(fileName: string, isDir: boolean): IconEntry {
  const key = getIconKey(fileName, isDir);
  return ICONS[key] || ICONS.unknown;
}

export function getIconSvg(fileName: string, isDir: boolean): string {
  const raw = getIconEntry(fileName, isDir).svg;
  // Remove width/height attributes from opening <svg> to allow scalable sizing
  return raw
    .replace(/(<svg[^>]*?)\s+width="[^"]*"/i, "$1")
    .replace(/(<svg[^>]*?)\s+height="[^"]*"/i, "$1");
}

export function registerIcon(key: string, entry: Partial<IconEntry>) {
  const normalized = key.toLowerCase();
  const existing = ICONS[normalized] || {
    key: normalized,
    svg: PLACEHOLDER_SVG(normalized),
  };
  ICONS[normalized] = {
    ...existing,
    ...entry,
    key: normalized,
    svg: entry.svg ?? existing.svg ?? PLACEHOLDER_SVG(normalized),
  };
}

export function bulkRegister(icons: Record<string, Partial<IconEntry>>) {
  for (const k of Object.keys(icons)) {
    registerIcon(k, icons[k]);
  }
}
